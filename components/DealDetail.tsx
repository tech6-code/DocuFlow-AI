...
export const DealDetail: React.FC<DealDetailProps> = ({ deals, salesSettings, onEdit, onDelete }) => {
    ...
    const handleDeleteDocument = async (doc: DealDocument) => {
        ...
        try {
            await dealService.deleteDealDocument(doc.id, doc.filePath);
            await loadDocuments();
            await loadHistory();
        } catch (error) {
    ...
    const handleAddFollowUp = async (followUpData: Omit<DealFollowUp, 'id' | 'dealId' | 'created'> | Partial<DealFollowUp>) => {
        ...
        try {
            if ('id' in followUpData && followUpData.id) {
                // Edit mode
                await dealService.updateDealFollowUp(followUpData.id as string, followUpData, currentUser.id);
            } else {
                // Create mode
    ...
    const handleDeleteFollowUp = async (followUpId: string) => {
        ...
        try {
            await dealService.deleteDealFollowUp(followUpId);
            await loadFollowUps();
            await loadHistory();
        } catch (error) {
    ...
    const handleAddNote = async (noteData: Omit<DealNote, 'id' | 'dealId' | 'created'> | Partial<DealNote>) => {
        ...
        try {
            if ('id' in noteData && noteData.id) {
                // Edit mode
                await dealService.updateDealNote(noteData.id as string, noteData, currentUser.id);
            } else {
                // Create mode
    ...
    const handleDeleteNote = async (noteId: string) => {
        ...
        try {
            await dealService.deleteDealNote(noteId);
            await loadNotes();
            await loadHistory();
        } catch (error) {

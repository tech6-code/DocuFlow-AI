        ...
        <div className="bg-muted/20 p-4 border-t border-border">
            <div className="overflow-x-auto">
                <table className="w-full text-sm border-collapse">
                    <thead>
                        <tr className="bg-muted text-muted-foreground text-[10px] uppercase tracking-widest font-semibold border-y border-border">
                            ...
                            <React.Fragment key={subCat}>
                                {category.category !== 'Equity' && (
                                    <tr className="bg-muted/30">
                                        <td colSpan={4} className={`px-4 py-2 font-bold text-xs text-muted-foreground border-b border-border italic`}>
                                            {subCat}
                                        </td>
                                    </tr>
                                )}
                                {items.map(({ account: acc, index }) => (
                                    <tr key={index} className="hover:bg-accent/5 border-b border-border last:border-0 group">
                                        <td className="py-2 px-4 text-foreground font-medium">
                                            <div className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    className="bg-transparent border-0 focus:ring-1 focus:ring-primary rounded px-1 py-0.5 w-full hover:bg-muted transition-colors outline-none"
                                                />
                                                <button
                                        ...
                                        <td className="py-2 px-4 text-center">
                                            <button
                                                ...
                                                className={`p-1.5 rounded-lg transition-all ${workingNotes && workingNotes[acc.name]?.length > 0 ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-primary hover:bg-muted'}`}
                                        ...
                                        <td className="py-1 px-2 text-right">
                                            <input
                                                ...
                                                className="w-full bg-muted border border-border rounded px-2 py-1.5 text-right font-mono text-foreground text-xs focus:ring-1 focus:ring-primary outline-none"
                                                placeholder="0"
                                            />
                                        </td>
                                        <td className="py-1 px-2 text-right">
        ...
        <div className="bg-card rounded-3xl border border-border shadow-2xl overflow-hidden">
            ...
            <div className="p-8 border-b border-border bg-muted/50">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <h3 className="text-2xl font-bold text-foreground tracking-tight uppercase">Adjust Opening Balance</h3>
                        <p className="text-muted-foreground mt-1">Review, adjust, and reconcile your opening balances before proceeding.</p>
                    </div>
                    <div className="flex items-center gap-3">
                        {onExport && (
                            <button onClick={onExport} className="flex items-center px-4 py-2 bg-muted hover:bg-muted/80 text-muted-foreground hover:text-foreground font-bold rounded-lg text-sm border border-border transition-all shadow-md">
                        ...
                        <input
                            ...
                            accept=".xls,.xlsx"
                        />
                        <button onClick={handleImportExcelClick} disabled={isExtracting} className="flex items-center px-4 py-2 bg-muted hover:bg-muted/80 text-foreground font-bold rounded-lg text-sm border border-border transition-all shadow-md disabled:opacity-50">
                        ...
                        <button onClick={() => setIsAddModalOpen(true)} className="px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-lg text-sm transition-all shadow-md">
                ...
                <div className="mt-6 flex flex-wrap items-center gap-6">
                    <div className="flex items-center space-x-4 bg-muted/30 p-2 rounded-lg border border-border">
                        <label htmlFor="migrationDate" className="text-xs font-bold text-muted-foreground flex items-center uppercase tracking-wider">
                        ...
                        <input
                            ...
                            className="bg-muted border border-border rounded-md text-foreground text-xs p-1.5 focus:ring-1 focus:ring-primary outline-none"
                        />
                    </div>
                ...
                {accountsData.map(cat => (
                    <div key={cat.category} className="group">
                        <button
                            ...
                            className={`w-full flex items-center justify-between p-4 transition-colors ${openCategory === cat.category ? 'bg-accent/10' : 'hover:bg-accent/5'}`}
                        >
                            <div className="flex items-center space-x-3">
                                <cat.icon className={`w-5 h-5 ${openCategory === cat.category ? 'text-foreground' : 'text-muted-foreground'}`} />
                                <span className="font-bold text-foreground uppercase tracking-wide">{cat.category}</span>
                                <span className="text-[10px] bg-muted text-muted-foreground font-mono px-2 py-0.5 rounded-full border border-border">{cat.accounts.length}</span>
                            </div>
                            {openCategory === cat.category ? <ChevronDownIcon className="w-5 h-5 text-muted-foreground" /> : <ChevronRightIcon className="w-5 h-5 text-muted-foreground" />}
                        </button>
            ...
            <div className="p-6 bg-muted/50 border-t border-border">
                ...
                <div className="flex justify-between mt-8 border-t border-border pt-6">
                    ...
                    <button onClick={handleContinue} disabled={!isBalanced && totalDebit > 0} className="px-8 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground font-extrabold rounded-xl shadow-xl disabled:opacity-50 transition-all">
            ...
            {noteModalOpen && (
                <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-card rounded-2xl border border-border shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-6 border-b border-border flex justify-between items-center bg-muted/50">
                            <h3 className="text-lg font-bold text-foreground uppercase tracking-wide">Working Notes: <span className="text-primary">{currentNoteAccount}</span></h3>
                            <button onClick={() => setNoteModalOpen(false)} className="text-muted-foreground hover:text-foreground transition-colors"><XMarkIcon className="w-5 h-5" /></button>
                        </div>
                        <div className="p-6">
                            <textarea
                                value={currentNoteText}
                                onChange={(e) => setCurrentNoteText(e.target.value)}
                                className="w-full h-40 bg-muted border border-border rounded-xl p-4 text-foreground text-sm focus:ring-2 focus:ring-primary outline-none resize-none font-mono leading-relaxed placeholder-muted-foreground/30"
                        ...
                        <div className="p-4 bg-muted/20 border-t border-border flex justify-end gap-3">
                            <button onClick={() => setNoteModalOpen(false)} className="px-4 py-2 text-muted-foreground hover:text-foreground font-bold text-sm">Cancel</button>
                            <button onClick={handleSaveNote} className="px-6 py-2 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-lg text-sm shadow-md transition-all">Save Notes</button>
                        </div>
                    </div>
                </div>
            )}

...
export const CtType2Results: React.FC<CtType2ResultsProps> = (props) => {
    ...
    }, [uniqueFiles, fileSummaries, editedTransactions, summaryFileFilter, manualBalances, getEffectiveConversionRate, persistedSummary]);
    ...
    const allFilesBalancesAed = useMemo(() => {
        ...
        return statementReconciliationData.reduce(
            (totals, recon) => {
                ...
                totals.closing += recon.closingBalanceAed || 0;
                return totals;
            },
            { opening: 0, closing: 0 }
        );
                ...
                case 1:
                    {
                        ...
                        stepData = { transactions: editedTransactions, summary: updatedSummary, manualBalances, conversionRates };
                    }
                    break;
                case 2:
    ...
    const STEP_NAMES_TYPE2 = [
        ...
        "Adjust Trial Balance", "Profit & Loss", "Balance Sheet", "Tax Computation",
        "LOU", "Signed FS and LOU Upload", "CT Questionnaire", "Generate Final Report"
    ];
    ...
    useEffect(() => {
        ...
        if (!isHydrated.current) {
            ...
            const highestSaved = Math.max(0, ...workflowData.filter((s: any) => s.stepNumber > 0).map((s: any) => s.stepNumber));
    ...
    const handleBulkRateConversion = useCallback((fileNames: string[], rateValue: string) => {
    ...
    useEffect(() => {
        if (ftaFormValues) {
            setReportForm((prev: any) => ({
                ...
                accountingIncomeTaxPeriod: Math.round(ftaFormValues.totalComprehensiveIncome || 0),
    ...
    const getType2TaxBaseFromPnl = useCallback(() => Math.round(
        ftaFormValues?.totalComprehensiveIncome
        || mappedPnLValues?.total_comprehensive_income
        || mappedPnLValues?.profit_loss_year
        || 0
    ), [ftaFormValues?.totalComprehensiveIncome, mappedPnLValues]);

    const computeType2TaxData = useCallback((source: Record<string, number> = taxComputationEdits): Record<string, number> => {
        const toInt = (v: unknown) => Math.round(Number(v) || 0);
        const isSbrClaimed = questionnaireAnswers[6] === 'Yes';
        const accountingBase = getType2TaxBaseFromPnl();
    ...
    const renderStep1 = () => {
        ...
        return (
            <div className="space-y-6">
                ...
                <div className="bg-card/60 backdrop-blur-xl rounded-2xl border border-border/50 p-2 mb-6">
                    <div className="flex flex-wrap items-center gap-2">
                        {/* Search */}
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="pl-9 pr-3 h-8 bg-card/40 border border-border/50 rounded-lg text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary/30 w-44 placeholder:text-muted-foreground font-medium"
                            />
                        </div>
                        ...
                        <CategoryDropdown
                            ...
                            className="!h-8 !bg-card/40 border border-border/50 !rounded-lg min-w-[130px] text-xs"
                            showAllOption={true}
                        />
                        ...
                        <select
                            ...
                            className="h-8 px-3 bg-card/40 border border-border/50 rounded-lg text-xs text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary/30 max-w-[130px] cursor-pointer font-bold"
                        >
                        ...
                        {(searchTerm || filterCategory !== 'ALL' || selectedFileFilter !== 'ALL') && (
                            <button
                                ...
                                className="h-8 px-2 text-[10px] font-black text-status-danger hover:bg-status-danger-soft rounded-lg transition-all uppercase tracking-widest"
                            >
                                Clear
                            </button>
                        )}
                        ...
                        <button
                            ...
                            className="h-8 px-3 bg-primary hover:bg-primary/90 text-primary-foreground text-[10px] font-black rounded-lg transition-all disabled:opacity-30 uppercase"
                        >
                            Apply
                        </button>
                        <button
                            ...
                            className="h-8 px-2 text-primary hover:bg-primary/10 text-[10px] font-black rounded-lg transition-all disabled:opacity-30 uppercase"
                        >
                        ...
                        <button
                            ...
                            disabled={selectedIndices.size === 0}
                            className="h-8 px-2 text-status-danger hover:bg-status-danger-soft text-[10px] font-black rounded-lg transition-all disabled:opacity-30 uppercase"
                        >
                        ...
                        <input
                            ...
                            className="h-8 px-3 bg-card/40 border border-border/50 rounded-lg text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-primary/30 w-24 placeholder:text-muted-foreground font-medium"
                        />
                        ...
                        <button
                            ...
                            className={`h-8 px-3 flex items-center gap-1.5 rounded-lg text-[10px] font-black transition-all border uppercase tracking-widest ${showPreviewPanel ? 'bg-primary border-primary/50 text-primary-foreground shadow-lg shadow-primary/10' : 'bg-card/40 border-border/50 text-muted-foreground hover:text-foreground hover:border-border'}`}
                        >
                            {showPreviewPanel ? <EyeSlashIcon className="w-3.5 h-3.5" /> : <EyeIcon className="w-3.5 h-3.5" />}
                            {showPreviewPanel ? 'Hide' : 'Preview'}
                        </button>

                        {isApplyingLocalRules && (
                            <span className="h-8 px-3 flex items-center text-[10px] font-black text-muted-foreground uppercase tracking-widest animate-pulse">
                                Applying...
                            </span>
                        )}
                        ...
                        <button
                            ...
                            className="h-8 px-4 bg-primary/10 hover:bg-primary/20 text-primary text-[10px] font-black rounded-lg flex items-center transition-all disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-widest border border-primary/20"
                        >
    ...
    const renderStep2Summarization = () => {
        ...
        return (
            <div className="space-y-6">
                                        ...
                                        <tr key={idx} className="hover:bg-muted/50">
                                            ...
                                            <td className="px-6 py-3 text-right font-mono text-primary/70">
                                                <div className="flex flex-col items-end">
                                                    {isAllFiles ? (
                                                        <span>{formatNumber(recon.openingBalanceAed)}</span>
                                                    ) : (
                                                        <input
                                                            type="text"
                                                    ...
                                                    {showDual && <span className="text-[10px] text-muted-foreground">Original: {formatNumber(recon.openingBalance)} {recon.currency}</span>}
                                                </div>
                                            </td>
                                            <td className="px-6 py-3 text-right font-mono text-status-danger">
                                                <div className="flex flex-col items-end">
                                                    <span>{formatNumber(isAllFiles ? recon.totalDebitAed : recon.totalDebit)} {isAllFiles ? 'AED' : recon.currency}</span>
                                                    {showDual && <span className="text-[10px] text-muted-foreground">Original: {formatNumber(recon.totalDebit)} {recon.currency}</span>}
                                                </div>
                                            </td>
                                            <td className="px-6 py-3 text-right font-mono text-status-success">
                                                <div className="flex flex-col items-end">
                                                    <span>{formatNumber(isAllFiles ? recon.totalCreditAed : recon.totalCredit)} {isAllFiles ? 'AED' : recon.currency}</span>
                                                    {showDual && <span className="text-[10px] text-muted-foreground">Original: {formatNumber(recon.totalCredit)} {recon.currency}</span>}
                                                </div>
                                            </td>
                                            <td className="px-6 py-3 text-right font-mono text-primary/80 font-bold">
                                                <div className="flex flex-col items-end">
                                                    <span>{formatNumber(isAllFiles ? recon.calculatedClosingAed : recon.calculatedClosing)} {isAllFiles ? 'AED' : recon.currency}</span>
                                                    {showDual && <span className="text-[10px] text-muted-foreground">Original: {formatNumber(recon.calculatedClosing)} {recon.currency}</span>}
                                                </div>
                                            </td>
                                            <td className="px-6 py-3 text-right font-mono text-foreground">
                                                <div className="flex flex-col items-end">
                                                    {isAllFiles ? (
                                                        <span>{formatNumber(recon.closingBalanceAed)}</span>
                                                    ) : (
                                                        <input
                                                            type="text"
                                                    ...
                                                    {showDual && <span className="text-[10px] text-muted-foreground">Original: {formatNumber(recon.closingBalance)} {recon.currency}</span>}
                                                </div>
                                            </td>
                                                ...
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    {recon.isManualExchangeRate && typeof recon.exchangeRate === 'number' && (
                                                        <span className="text-[10px] text-primary/80">
                                                            1 {recon.currency} = {formatNumber(recon.exchangeRate)} AED
                                                        </span>
                                                    )}
                                                </div>
                                            </td>
                ...
                <WorkflowNavigation onBack={handleBack} onNext={handleConfirmSummarization} nextLabel="Confirm & Continue" />
            </div>
        );
    };
    ...
    const renderStep4InvoiceSummarization = () => (
        <div className="space-y-6">
            ...
            />
            <WorkflowNavigation onBack={handleBack} onNext={handleContinueToReconciliation} nextLabel="Confirm & Continue" />
        </div>
    );
    ...
    const renderStep6VatAdditionalDocs = () => (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <WorkflowNavigation
                ...
                nextLabel={isExtracting ? "Extracting VAT Data..." : "Extract & Continue"}
    ...
    const renderStep7VatSummarization = () => {
        ...
        return (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <WorkflowNavigation
                    onBack={handleBack}
                    onNext={handleContinueToOpeningBalances}
                    nextLabel="Confirm & Continue"
                    rightContent={
                        <div className="flex gap-4">
                            <button
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                <div className="p-6 bg-background border-t border-border">
                    ...
                    <div className="mt-8 border-t border-border pt-6">
                        <WorkflowNavigation
                            onBack={handleBack}
                            onNext={handleOpeningBalancesComplete}
                            nextLabel="Continue"
                            nextDisabled={Math.abs(variance) >= 1}
                            rightContent={
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                ...
                <div className="p-6 bg-background border-t border-border">
                    ...
                    <div className="mt-8 border-t border-border pt-6">
                        <WorkflowNavigation
                            onBack={handleBack}
                            onNext={handleContinueToProfitAndLoss}
                            nextLabel="Continue"
                            nextDisabled={Math.abs(variance) >= 1}
                            rightContent={
    ...
    const handleExportTaxComputation = () => {
        ...
        if (taxSummary) {
            taxSummary.fields.forEach((f: any) => {
                if (f.type !== 'header') {
                    sheetData.push([f.label, computedTaxData[f.field] ?? 0]);
                }
            });
        }
    ...
    const renderStep13LOU = () => (
        <div className="space-y-6 max-w-5xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-card rounded-2xl border border-border shadow-2xl overflow-hidden ring-1 ring-border">
                ...
                <div className="p-8 bg-background border-t border-border">
                    <WorkflowNavigation onBack={handleBack} onNext={handleContinueToSignedFsLouUpload} nextLabel="Confirm & Continue" />
                </div>
            </div>
        </div>
    );
    ...
    const renderStep14SignedFsLouUpload = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <WorkflowNavigation
                ...
                showSkip
                onSkip={handleContinueToQuestionnaire}
                skipLabel="Skip"
            />
        </div>
    );
    ...
    const renderStep16FinalReport = () => {
        ...
        return (
            <div className="space-y-6 max-w-5xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                <WorkflowNavigation onBack={handleBack} isLastStep />
            </div>
        );
    };

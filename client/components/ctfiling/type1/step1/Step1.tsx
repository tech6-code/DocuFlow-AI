...
export const Step1: React.FC = () => {
    ...
    // Filter logic
    const filteredTransactions = editedTransactions.filter(t => {
        // Search Filter
        const matchesSearch = !searchTerm ||
            (typeof t.description === 'string' && t.description.toLowerCase().includes(searchTerm.toLowerCase())) ||
            (typeof t.description === 'object' && JSON.stringify(t.description).toLowerCase().includes(searchTerm.toLowerCase()));

        // Category Filter
        const matchesCategory = filterCategory === 'ALL' || t.category === filterCategory ||
            (filterCategory === 'UNCATEGORIZED' && (!t.category || t.category.toLowerCase().includes('uncategorized')));
                ...
                <div className="bg-red-900/40 border border-red-500/50 rounded-xl p-4 flex items-start gap-4 animate-pulse">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-red-200/70 text-xs leading-relaxed">
                            ...
                            <br />
                            {selectedFileFilter === 'ALL' && balanceValidation.anyInvalid && (
                                <>

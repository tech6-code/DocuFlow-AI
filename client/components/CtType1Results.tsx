...
export const CtType1Results: React.FC<CtType1ResultsProps> = ({
    ...
    const STEP_NAMES_TYPE1 = [
        "Review Categories", "Summarization", "VAT Docs Upload", "VAT Summarization",
        "Opening Balances", "Adjust Trial Balance", "Profit & Loss", "Balance Sheet",
        "Tax Computation", "LOU", "Signed FS and LOU Upload", "CT Questionnaire", "Generate Final Report"
    ];
    ...
    const handleStep4FileSelected = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (importedVatFileResults.length > 0) {
                ...
                await handleSaveStep(3, { additionalDetails: { vatFileResults: importedVatFileResults } }, 'completed');
    ...
    const handleConfirmAdditionalStatements = useCallback(() => {
        ...
        const appended = pendingAdditionalStatementDrafts.flatMap(draft => {
            ...
            if (draft.previewUrls.length) {
                previewEntries[resolvedTargetFile] = [
                    ...(previewEntries[resolvedTargetFile] || []),
                    ...draft.previewUrls
                ];
            }
    ...
    const syncType1TaxToStatements = (taxData: Record<string, number>) => {
        ...
        const prevYearTaxProvision = Math.round(Number(pnlValues['provisions_corporate_tax']?.previousYear || 0));

        setPnlValues(prev => {
            const profitBeforeTax = Number(prev['profit_loss_year']?.currentYear || 0);
            const previousProfitAfterTax = Number(prev['profit_after_tax']?.previousYear || 0);
            return {
                ...prev,
                provisions_corporate_tax: {
                    currentYear: taxLiability,
                    previousYear: Number(prev['provisions_corporate_tax']?.previousYear || 0)
                },
                profit_after_tax: {
                    currentYear: Math.round(profitBeforeTax - taxLiability),
                    previousYear: previousProfitAfterTax
                }
            };
        });
    ...
    const handleContinueToSignedFsLouUpload = async () => {
        await handleSaveStep(10, { louData }, 'completed');
        isManualNavigationRef.current = true;
        setCurrentStep(11);
    };
            ...
            <div className="space-y-6">
                ...
                <div className="flex justify-between items-center bg-card p-4 rounded-xl border border-border">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={handleImportStep1} className="px-3 py-2 border border-border text-foreground rounded-lg hover:border-border hover:text-foreground transition-colors text-sm">
                            Import Data
                        </button>
                        <input
    ...
    const renderStepSummarization = () => (
        <div className="space-y-6">
            ...
            <WorkflowNavigation
                onBack={handleBack}
                onNext={handleSummarizationContinue}
                nextLabel="Confirm & Continue"
                nextDisabled={editedTransactions.length === 0}
            />
        </div>
    );
    ...
    const renderStep3VatDocsUpload = () => (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <WorkflowNavigation
                onBack={handleBack}
                onNext={additionalFiles.length === 0 && additionalDetails.vatFileResults && additionalDetails.vatFileResults.length > 0
                    ? () => { isManualNavigationRef.current = true; setCurrentStep(4); }
                    : handleExtractAdditionalData}
                nextLabel={additionalFiles.length === 0 && additionalDetails.vatFileResults && additionalDetails.vatFileResults.length > 0
                    ? "Continue to Summarization"
                    : isExtracting ? "Extracting VAT Data..." : "Extract & Continue"}
                nextDisabled={!(additionalFiles.length === 0 && additionalDetails.vatFileResults && additionalDetails.vatFileResults.length > 0) && (additionalFiles.length === 0 || isExtracting)}
                            ...
                            <table className="w-full text-center">
                                <thead className="text-xs font-black uppercase tracking-widest text-muted-foreground border-b border-border">
                                    <tr>
                                        ...
                                        <th className="py-4 px-4 text-right">Standard Rated</th>
                                        <th className="py-4 px-4 text-right text-primary">VAT Amount</th>
                                        <th className="py-4 px-4 text-right bg-muted/20 text-foreground">Total Purchases</th>
                                    </tr>
                                </thead>
                                <tbody className="text-foreground/80 text-sm font-mono">
                                    {periods.map((p: any) => {
                                    ...
                                    <tr className="bg-muted/40 font-bold border-t-2 border-border">
                                        <td className="py-5 px-4 text-left font-black text-foreground text-sm uppercase italic">Purchases Total</td>
                                        ...
                                        <td className="py-5 px-4 text-right text-primary text-sm">{formatDecimalNumber(grandTotals.purchases.vat)}</td>
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm">
                                                                ...
                                                                <td className={`py-2 pr-4 text-foreground/80 font-medium ${padding}`}>
                                                                    <div className="flex items-center justify-between group">
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            className={`p-1.5 rounded transition-all ${item.hasBreakdown ? 'text-primary bg-primary/20/20 opacity-100 shadow-inner' : 'text-muted-foreground hover:text-primary-foreground hover:bg-muted/80 opacity-0 group-hover:opacity-100'}`}
                ...
                <div className="p-6 bg-background/80 border-t border-border shadow-inner">
                    ...
                    <WorkflowNavigation
                        ...
                        nextLabel="Continue"
                        nextDisabled={Math.abs(grandTotal.debit - grandTotal.credit) > 0.01}
                        rightContent={
                            <button
        ...
        <div className="space-y-6 max-w-5xl mx-auto pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-card rounded-2xl border border-border shadow-2xl overflow-hidden ring-1 ring-border">
                ...
                <div className="p-8 bg-background border-t border-border">
                    <WorkflowNavigation
                        onBack={handleBack}
                        onNext={handleContinueToSignedFsLouUpload}
                        nextLabel="Confirm and Proceed"
                    />
                </div>
            </div>
        </div>
            ...
            {showVatFlowModal && createPortal(
                <div className="fixed inset-0 bg-background/90 backdrop-blur-md z-[100000] flex items-center justify-center p-4 animate-in fade-in zoom-in duration-300">
                    <div className="bg-background rounded-3xl border border-border shadow-2xl w-full max-w-lg overflow-hidden relative group">
                        <div className="absolute inset-0 bg-muted/40 opacity-50 pointer-events-none" />

                        <div className="p-10 text-center relative z-10">
                            <div className="w-20 h-20 bg-primary/20/20 rounded-full flex items-center justify-center mx-auto mb-6 border border-primary/50/30 shadow-md animate-pulse">
                            ...
                            <div className="flex flex-col sm:flex-row justify-center gap-4">
                                ...
                                <button
                                    ...
                                    className="px-10 py-4 bg-primary hover:bg-primary/90 text-primary-foreground font-black rounded-2xl shadow-xl shadow-primary/20 transition-all uppercase text-xs tracking-widest transform hover:-translate-y-1 w-full sm:w-auto flex items-center justify-center gap-2 group/btn"
                                >
                                    <CheckIcon className="w-4 h-4" />
                                    Yes, I have them
                                </button>
                        ...
                        <div className="h-1.5 w-full bg-muted/40 bottom-0 absolute" />
                    </div>
                </div>,
                document.body
            )}
                            ...
                            <table className="w-full text-sm text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-border">
                                    ...
                                    {(() => {
                                        ...
                                        return (
                                            <tr className="bg-primary/10 border-l-4 border-primary/50/50 group/base">

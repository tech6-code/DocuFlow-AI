...
const DetailItem: React.FC<DetailItemProps> = ({ label, value, icon: Icon }) => (
    <div className="bg-muted/40 p-3 rounded-xl border border-border flex items-start space-x-3">
        <div className="p-2 bg-card/50 rounded-lg text-muted-foreground">
...
export const DealViewModal: React.FC<DealViewModalProps> = ({ isOpen, onClose, deal, salesSettings, onEdit }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl bg-card rounded-2xl border border-border shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                {/* Header */}
                <div className="p-6 border-b border-border flex justify-between items-center bg-card/50">
                    <div>
                        <div className="flex items-center space-x-2 mb-1">
                            <h2 className="text-xl font-bold text-foreground">{deal.companyName}</h2>
                    ...
                    <div className="flex space-x-2">
                        ...
                        <button
                            onClick={onClose}
                            className="p-2 bg-muted text-muted-foreground rounded-full hover:bg-muted/80 hover:text-foreground transition-colors"
                        >
                ...
                <div className="p-6 overflow-y-auto max-h-[70vh] custom-scrollbar">
                    ...
                    {/* AI Insights Section */}
                    <div className="mb-8 bg-gradient-to-br from-primary/10 to-accent/10 rounded-xl border border-primary/20 p-5 relative overflow-hidden group">
                        ...
                        <div className="flex justify-between items-start mb-4 relative z-10">
                            ...
                            <button
                                ...
                                className="text-xs flex items-center px-3 py-1.5 rounded-lg bg-primary/20 text-primary hover:bg-primary/30 transition-colors disabled:opacity-50"
                            >
                                {isAnalyzing ? (
                                    <>Analyzing...</>
                                ) : (
                        ...
                        {aiAnalysis ? (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
                                <div className="md:col-span-1 flex flex-col items-center justify-center p-4 bg-card/40 rounded-lg border border-primary/20">
                ...
                <div className="p-4 border-t border-border flex justify-end bg-card/30">
                    <button
                        onClick={onClose}
                        className="px-6 py-2 bg-muted text-muted-foreground font-semibold rounded-lg hover:bg-muted/80 transition-colors text-sm border border-border"
                    >
                        Close
                    </button>
                </div>
